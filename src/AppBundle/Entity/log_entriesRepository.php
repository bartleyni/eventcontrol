...
class log_entriesRepository extends EntityRepository
{
    ...
    {
        if ($location){
            return $this->getEntityManager()->createQuery('SELECT log_entry.location DISTINCT(log_entry.location) FROM AppBundle\Entity\log_entries log_entry WHERE log_entry.event = :event AND log_entry.location LIKE :location')->setParameter('event', $event)->setParameter('location', '%'.$location.'%')->getResult();
        } else {
            return $this->getEntityManager()->createQuery('SELECT log_entry.location DISTINCT(log_entry.location) FROM AppBundle\Entity\log_entries log_entry WHERE log_entry.event = :event')->setParameter('event', $event)->getResult();
        }
    }
