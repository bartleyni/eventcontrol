    ...
    public function map_json_data($filter=null, $filter_type=null)
    {
        ...
        foreach ($logs as $log)
        {
            if($log['latitude'] != null)
            {
                ...
                $key = array_search(round($log['latitude'], 4).", ".round($log['longitude'], 4), array_column($markers, 'latlong'));
                
                if($key == null)
                {
                    $markerId = $markerId+1;
                    $markers[$markerId] = ['latlong' => round($log['latitude'], 4).", ".round($log['longitude'], 4), 'latitude' => $log['latitude'], 'longitude' => $log['longitude']];
                    $key = $markerId;
